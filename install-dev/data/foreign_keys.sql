...
#TODO: find out if customer and guest foreign keys are possible
ALTER TABLE `PREFIX_cart`
  ADD FOREIGN KEY `fk_cart_shop_group`(`id_shop_group`)
REFERENCES `PREFIX_shop_group` (`id_shop_group`)
  ON DELETE CASCADE;
ALTER TABLE `PREFIX_cart`
  ADD FOREIGN KEY `fk_cart_shop`(`id_shop`)
REFERENCES `PREFIX_shop` (`id_shop`)
  ON DELETE CASCADE;
ALTER TABLE `PREFIX_cart`
  ADD FOREIGN KEY `fk_cart_carrier`(`id_carrier`)
REFERENCES `PREFIX_carrier` (`id_carrier`)
  ON DELETE CASCADE;
ALTER TABLE `PREFIX_cart`
  ADD FOREIGN KEY `fk_cart_lang`(`id_lang`)
REFERENCES `PREFIX_lang` (`id_lang`)
  ON DELETE CASCADE;
ALTER TABLE `PREFIX_cart`
  ADD FOREIGN KEY `fk_cart_address_delivery`(`id_address_delivery`)
REFERENCES `PREFIX_address` (`id_address`)
  ON DELETE CASCADE;
ALTER TABLE `PREFIX_cart`
  ADD FOREIGN KEY `fk_cart_address_invoice`(`id_address_invoice`)
REFERENCES `PREFIX_address` (`id_address`)
  ON DELETE CASCADE;
ALTER TABLE `PREFIX_cart`
  ADD FOREIGN KEY `fk_cart_shop_group`(`id_shop_group`)
...
ALTER TABLE `PREFIX_cart_rule`
  ADD FOREIGN KEY `fk_cart_rule_customer`(`id_customer`)
REFERENCES `PREFIX_customer` (`id_customer`)
  ON DELETE CASCADE;

ALTER TABLE `PREFIX_cart_rule_lang`
  ADD FOREIGN KEY `fk_cart_rule_lang_lang`(`id_lang`)
REFERENCES `PREFIX_lang` (`id_lang`)
  ON DELETE CASCADE;

ALTER TABLE `PREFIX_cart_rule_country`
  ADD FOREIGN KEY `fk_cart_rule_country_country`(`id_country`)
REFERENCES `PREFIX_country` (`id_country`)
  ON DELETE CASCADE;

ALTER TABLE `PREFIX_cart_rule_group`
  ADD FOREIGN KEY `fk_cart_rule_group_group`(`id_group`)
REFERENCES `PREFIX_group` (`id_group`)
  ON DELETE CASCADE;

#TODO: find out why the carrier's id_reference is not used
# ALTER TABLE `PREFIX_cart_rule_carrier`
#   ADD FOREIGN KEY `fk_cart_rule_carrier_carrier`(`id_carrier`)
# REFERENCES `PREFIX_carrier` (`id_carrier`)
#   ON DELETE CASCADE;

ALTER TABLE `PREFIX_cart_rule_combination`
  ADD FOREIGN KEY `fk_cart_rule_combination_cart_rule_1`(`id_cart_rule_1`)
REFERENCES `PREFIX_cart_rule` (`id_cart_rule`)
  ON DELETE CASCADE;
ALTER TABLE `PREFIX_cart_rule_combination`
  ADD FOREIGN KEY `fk_cart_rule_combination_cart_rule_2`(`id_cart_rule_2`)
REFERENCES `PREFIX_cart_rule` (`id_cart_rule`)
  ON DELETE CASCADE;

ALTER TABLE `PREFIX_cart_rule_product_rule_group`
  ADD FOREIGN KEY `fk_cart_rule_product_rule_group_cart_rule`(`id_cart_rule`)
REFERENCES `PREFIX_cart_rule` (`id_cart_rule`)
  ON DELETE CASCADE;

ALTER TABLE `PREFIX_cart_rule_product_rule_value`
  ADD FOREIGN KEY `fk_cart_rule_product_rule_value_product_rule`(`id_product_rule`)
REFERENCES `PREFIX_product_rule` (`id_product_rule`)
  ON DELETE CASCADE;

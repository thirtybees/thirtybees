...
var priceDatabasePrecision = {$smarty.const._TB_PRICE_DATABASE_PRECISION_};
    // Script to load the customers for edit Specific Prices.
    var Customer_m = new Object();
    $(document).ready(function () {
        Customer_m = {
            "hiddenField": jQuery('#spm_id_customer'),
            "field": jQuery('#customer_m'),
            "container": jQuery('#customers_m'),
            "loader": jQuery('#customerLoader'),
            "init": function () {
                jQuery(Customer_m.field).typeWatch({
                    "captureLength": 1,
                    "highlight": true,
                    "wait": 50,
                    "callback": Customer_m.search
                }).focus(Customer_m.placeholderIn).blur(Customer_m.placeholderOut);
            },
            ...
            "search": function ()
            {
                ...
                jQuery.ajax({
                    ...
                    "data": {
                        ...
                        "customer_search": Customer_m.field.val()
                    },
                    "success": Customer_m.success
                });
            },
            "success": function (result)
            {
                if (result.found) {
                    ...
                    jQuery.each(result.customers, function () {
                        html += '<li><a class="fancybox" href="{$link->getAdminLink('AdminCustomers')}&id_customer=' + this.id_customer + '&viewcustomer_m&liteDisplaying=1">' + this.firstname + ' ' + this.lastname + '</a>' + (this.birthday ? ' - ' + this.birthday : '');
                        html += ' - ' + this.email;
                        html += '<a onclick="Customer_m.select(' + this.id_customer + ', \'' + this.firstname + ' ' + this.lastname + '\'); return false;" href="#" class="btn btn-default">{l s='Choose'}</a></li>';
                    });
                    html += '</ul>';
                } else
                    html = '<div class="alert alert-warning">{l s='No customers found'}</div>';
                Customer_m.hideLoader();
                Customer_m.container.html(html);
                jQuery('.fancybox', Customer_m.container).fancybox();
            },
            "select": function (id_customer, fullname)
            {
                Customer_m.hiddenField.val(id_customer);
                Customer_m.field.val(fullname);
                Customer_m.container.empty();
                return false;
            },
            "showLoader": function () {
                Customer_m.loader.fadeIn();
            },
            "hideLoader": function () {
                Customer_m.loader.fadeOut();
            }
        };
        Customer_m.init();
    });
...
<div class="panel">
		...
		<div class="col-lg-12">
			...
			<a class="btn btn-default" href="#" id="hide_specific_price" style="display:none">
				<i class="icon-remove text-danger"></i> {l s='Cancel new specific price'}
                         ...
                         <!--Button to hide the EditSpecificPrices form. -->
                        <a class="btn btn-default" href="#" id="hide_edit_specific_price" style="display:none">
	...
	<script type="text/javascript">
		...
		{/foreach}
                ...
                <!-- The Edit Specific Price form. Only show when the edit button is clicked. -->
                <div id="edit_specific_price" class="well clearfix" style="display: none;">
                    <div class="alert alert-warning">
                        {l s='Keep in mind! When the specific price reduction type is "percentage" and the value is not changed, the reduction amount will be divided by 100.'}
                    </div>
                    <div class="col-lg-12">
                        <div class="form-group">
                            <label class="control-label col-lg-2" for="{if !$multi_shop}spm_currency_0{else}spm_id_shop{/if}">{l s='For'}</label>
                            <div class="col-lg-9">
                                <div class="row">
                                    {if !$multi_shop}
                                    ...
                                    {/if}
                                    <div class="col-lg-3">
                                        <select name="spm_id_currency" id="spm_currency_1" onchange="changeCurrencySpecificPrice(0);">
                        ...
                        <div class="form-group">
                            <label class="control-label col-lg-2" for="customer_m">{l s='Customer'}</label>
                            <div class="col-lg-4">
                                <input type="hidden" name="spm_id_customer" id="spm_id_customer" value="0" />
                                <div class="input-group">
                                    <input type="text" name="customer_m" value="{l s='Enter customer details'}" id="customer_m" autocomplete="off" />
                            ...
                            <p class="button">
                                <label class="control-label col-lg-2" for="dc">
                                    <span class="label-tooltip" data-toggle="tooltip" title="{l s='You can press this button to set the specific price to all customers.'}">{l s='All Customers'}</span>
                                </label>
                                <a type="button" class="btn btn-default" id="AllCustomers" name="dc" onclick="AllCustomers(0);"><i class="icon-pencil"></i></a>
                            </p>
                        ...
                        {if $combinations|@count != 0}
                            <div class="form-group">
                                <label class="control-label col-lg-2" for="spm_id_product_attribute">{l s='Combination:'}</label>
                                <div class="col-lg-4">
                        ...
                        {/if}
                        <!-- Is required in order to update the specific price -->
                        <input type="hidden" name="spm_id_specific_price" id="spm_id_specific_price" value="">
                        <div class="form-group">
                            <label class="control-label col-lg-2" for="spm_from">{l s='Available'}</label>
                            <div class="col-lg-9">
                                <div class="row">
                                    <div class="col-lg-4">
                                        <div class="input-group">
                                            <span class="input-group-addon">{l s='from'}</span>
                                            <input type="text" name="spm_from" class="datepicker" value="" style="text-align: center" id="spm_from" />
                                    ...
                                    <div class="col-lg-4">
                                        <div class="input-group">
                                            ...
                                            <input type="text" name="spm_to" class="datepicker" value="" style="text-align: center" id="spm_to" />
                                            <span class="input-group-addon"><i class="icon-calendar-empty"></i></span>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="form-group">
                            <label class="control-label col-lg-2" for="spm_from_quantity">{l s='Starting at'}</label>
                            <div class="col-lg-4">
                                <div class="input-group">
                                    <span class="input-group-addon">{l s='unit'}</span>
                                    <input type="text" name="spm_from_quantity" id="spm_from_quantity" value="1" />
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="control-label col-lg-2" for="spm_price">{l s='Product price'}
                                {if $country_display_tax_label}
                                    {l s='(tax excl.)'}
                                {/if}
                            </label>
                            <div class="col-lg-9">
                                <div class="row">
                                    <div class="col-lg-4">
                                        <div class="input-group">
                                            <span class="input-group-addon">{$currency->prefix}{$currency->suffix}</span>
                                            <input type="text" name="spm_price" id="spm_price" value="{$product->price|string_format:$priceDisplayPrecisionFormat}" />
                                        </div>
                                    </div>
                                    <p class="button">
                                        <label class="control-label col-lg-2" for="default_price">
                                            <span class="label-tooltip" data-toggle="tooltip" title="{l s='Set the fixed price back to the default price.'}">{l s='Default price'}</span>
                                        </label>
                                        <a type="button" class="btn btn-default" id="default_price" name="no" onclick="DefaultPrice(-1.000000);"><i class="icon-pencil"></i></a>
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="control-label col-lg-2" for="spm_reduction">{l s='Apply a discount of'}</label>
                            <div class="col-lg-4">
                                <div class="row">
                                    <div class="col-lg-3">
                                        <input type="text" name="spm_reduction" id="spm_reduction" value="0.00"/>
                                    </div>
                                    <div class="col-lg-6">
                                        <select name="spm_reduction_type" id="spm_reduction_type">
                                    ...
                                    <div class="col-lg-3">
                                        <select name="spm_reduction_tax" id="spm_reduction_tax">
                    ...
                    <td><input type="hidden" class="btn btn-primary" name="submit" value="update"></td>
                </div>
		...
		<div class="col-lg-12">
            ...
            $(document).ready(function () {
                ...
                $('#id_product_attribute').change(function () {
                    $('#spm_current_ht_price').html(product_prices[$('#id_product_attribute option:selected').val()]);
                });
                ...
                $('#spm_reduction_type').on('change', function () {
                    if (this.value == 'percentage')
                        $('#spm_reduction_tax').hide();
                    else
                        $('#spm_reduction_tax').show();
                });
            });
            
            // DefaultPrice function sets the "spm_price" text field back to the default price.
            function DefaultPrice(p) {
                document.getElementById("spm_price").value = p;
            }
            
            function AllCustomers(c) {
                document.getElementById("spm_id_customer").value = c;
            }
        </script>
	<script type="text/javascript">
		var currencyName = '{$currency->name|escape:'html':'UTF-8'|@addcslashes:'\''}';

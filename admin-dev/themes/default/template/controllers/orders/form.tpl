  ...
  var request = null;
			...
			success : function(res)
			{
				if(res.found)
				{
					...
					$.each(res.orders, function() {
						html_orders += '<tr>';
						html_orders += '<td>'+this.id_order+'</td><td>'+this.date_add+'</td><td>'+(this.nb_products ? this.nb_products : '0')+'</td><td>'+this.totalPaid+'</span></td><td>'+this.payment+'</td><td>'+this.order_state+'</td>';
		...
		request = $.ajax({
			...
			success : function(res) {
				...
				if(res.found)
				{
					...
					$('#products_found').show();
					products_found += '<label class="control-label col-lg-3">{l s='Product'}</label><div class="col-lg-6"><select id="id_product" onclick="display_product_attributes();display_product_customizations();">';
	...
	function updateCartProducts(products, gifts, id_address_delivery)
	{
		...
		$.each(products, function() {
			...
			cart_content += '<tr><td width="10%"><img src="'+this.image_link+'" title="'+this.name+'" width="100%" /></td><td>'+this.name+'<br />'+this.attributes_small+'</td><td>'+this.reference+'</td><td><input type="text" rel="'+this.id_product+'_'+this.id_product_attribute+'" class="product_unit_price" value="' + this.numeric_price + '" /></td><td>';

...
<script type="text/javascript">
  ...
  var priceDatabasePrecision = {$smarty.const._TB_PRICE_DATABASE_PRECISION_};
	...
	function searchProducts()
	{
		...
		var product_search_value = $('#product').val();
		...
		$.ajax({
			...
			data : {
				...
				product_search: product_search_value},
			success : function(res) {

				// Make sure, that the result is only rendered, if the input value wasn't changed by the user afterwards
				if (product_search_value === $('#product').val()) {

...
{/if}
	...
	{block name="filtersBlock"}
		<div id="list-filters-{$list_id}" style="display:{if $showFilters}block{else}none{/if}">
			<div class="panel col-lg-12">
				<div class="panel-body">
					...
					<div id="no-filters-placeholder" {if $filters}style="display:none"{/if}>
						<div class="alert alert-info">{l s='No filters'}</div>
					</div>
				</div>
				<div class="panel-footer">
					<a class="btn btn-default" href="#" onclick="return newListFilter('{$list_id}')">
						<i class="icon-plus"></i>&nbsp;{l s='Add filter'}
					</a>
					<button type="submit" name="submitFilter" class="btn btn-primary" data-list-id="{$list_id}">
		...
		<script>
			...
			renderListFilters('{$list_id}', {$filters|json_encode});
		</script>
	{/block}
	{/if}

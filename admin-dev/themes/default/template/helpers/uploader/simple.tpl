...
{elseif isset($max_files) && $files|count >= $max_files}
  ...
  <div class="form-group">
    <div class="col-sm-6">
      {if !ini_get('file_uploads')}
        <div class="alert alert-danger">{l s='File uploads have been turned off. Please ask your webhost to enable file uploads (%s).' sprintf=['<code>file_uploads = on</code>']}</div>
      {else}
        <input id="{$id|escape:'html':'UTF-8'}" type="file"
               name="{$name|escape:'html':'UTF-8'}{if isset ($multiple) && $multiple}[]{/if}"{if isset($multiple) && $multiple} multiple="multiple"{/if}
               class="hide"/>
        <div class="dummyfile input-group">
          <span class="input-group-addon"><i class="icon-file"></i></span>
          <input id="{$id|escape:'html':'UTF-8'}-name" type="text" name="filename" readonly/>
          <span class="input-group-btn">
          <button id="{$id|escape:'html':'UTF-8'}-selectbutton" type="button" name="submitAddAttachments"

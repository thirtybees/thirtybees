...
class AdminControllerCore extends Controller implements DatasetQuery
{
    ...
    public function processFilter()
    {
        ...
        $prefix = $this->getControllerListPrefix();
                ...
                if (isset($filterFields[$fieldId])) {
                    ...
                    for ($i = 0; $i < $operatorNum; $i++) {
                        if (isset($adhocFilter['args'][$i])) {
                           $operands[] = $valueType->deserializeValue($adhocFilter['args'][$i]);
                        } else {
                            break;
                        }
                    }
        ...
        // save filters
        $this->processFilterData($_POST, $filterFields, $listView);
        $this->processFilterData($_GET, $filterFields, $listView);

        // process order by and order way
        $this->processOrderByParameter($listView);
        $this->processOrderWayParameter($listView);

        // save list view
        $listViewStorage->saveListView($listView);
     ...
     * @return string
     */
    protected function getControllerListPrefix()
    {
     ...
     * @return void
     */
    protected function processFilterData(array $array, array $filterFields, ListView $listView)
    {
        $filterFieldsPrefix = $this->list_id . 'Filter_';
        ...
        foreach ($array as $key => $value) {
            ...
            if (stripos($key, $filterFieldsPrefix) === 0) {
                $filterId = substr($key, strlen($filterFieldsPrefix));
                if (isset($filterFields[$filterId])) {
                    $filter = $this->constructColumnFilter($filterFields[$filterId], $value);
                    if ($filter) {
                        $listView->addFilter($filter);
                    } else {
                        $listView->removeFilterById($filterId);
                    }
                } else {
                    trigger_error("Failed to find filter field for filter '$filterId'", E_USER_WARNING);
                }
            }
        }
    }
     ...
     * @return void
     */
    public function processOrderByParameter(ListView $listView): void
    {
        if (Tools::isSubmit($this->list_id . 'Orderby')) {
            $value = Tools::getValue($this->list_id . 'Orderby');
            if (Validate::isOrderBy($value)) {
                $listView->setOrderBy($value);
            } else {
                $listView->setOrderBy(null);
            }
        }
    }
     ...
     * @return void
     */
    public function processOrderWayParameter(ListView $listView): void
    {
        if (Tools::isSubmit($this->list_id . 'Orderway')) {
            $value = Tools::getValue($this->list_id . 'Orderway');
            if (Validate::isOrderWay($value)) {
                $listView->setOrderWay($value);
            } else {
                $listView->setOrderWay(null);
            }
        }
    }
...
//            $pathToImage = false;
    ...
    public function getList(
        ...
        $orderBy = $this->resolveOrderBy($orderBy, $listView);
        $orderWay = $this->resolveOrderWay($orderWay, $listView);
    ...
    public function setHelperListDisplay(HelperList $helper)
    {
        ...
        $helper->setListView($this->getListView());
    }
     ...
     * @return string|null
     */
    protected function resolveOrderBy(?string $orderBy, ListView $listView)
    {
        if (! empty($orderBy)) {
            return $orderBy;
        }
        $orderBy = $listView->getOrderBy();
        if ($orderBy) {
            return $orderBy;
        }
        if ($this->_orderBy) {
     ...
     * @return string|null
     */
    protected function resolveOrderWay(?string $orderWay, ListView $listView)
    {
        if (! empty($orderWay)) {
            return $orderWay;
        }
        $orderWay = $listView->getOrderWay();
        if ($orderWay) {
            return $orderWay;
        }
        if ($this->_orderWay) {
    ...
    protected function resolveValueType(string $fieldId, array $listField): ValueType
    {
        ...
        switch ($type) {
            ...
            case HelperList::COLUMN_TYPE_EDITABLE:
    ...
    {
        return ServiceLocator::getInstance()->getListViewStorage();
    }
     ...
     * @throws PrestaShopException
     */
    protected function getListView(): ListView
    {
        $listId = $this->list_id ?? $this->table;
        if (! isset($this->listViews[$listId])) {
            $namespace = $this->getControllerListPrefix();
            $filterFields = $this->getListAvailableFilterFields();
            $listView = $this->getListViewStorage()->getListView($namespace, $listId, $filterFields);
            $this->listViews[$listId] = $listView;
        }
        return $this->listViews[$listId];
    }
    ...
    protected function shouldProcessListFilter(): bool
    {
        ...
        $filters = $this->getListView()->getFilters();
        return !!$filters;
    }
     ...
     * @return Filter|null
     */
    protected function constructColumnFilter(FilterField $filterField, $value): ?Filter
    {
        $valueType = $filterField->getValueType();
        if ($valueType->getType() == DatetimeValueType::TYPE_DATETIME) {
            // column filters can't handle datetime correctly, ui element only displays date portion
            $valueType = DatetimeValueType::date();
        }
        $operator = $valueType->getDefaultOperator();
        $serialized = $this->serializeListFilterValue($value);
        if ($serialized === '') {
            return null;
        }
        $operands = $operator->deserializeOperands($valueType, $serialized);
        if (! $operands) {
            trigger_error("Failed to deserialize filter operands: $serialized", E_USER_WARNING);
            return null;
        }
        return new Filter(
            Filter::TYPE_COLUMN,
     ...
     * @return int
     */
    protected function resolvePaginationStart(int $pageSize): int
    {
        if ($pageSize === 0) {
            return 0;
        }
        ...
        if (Tools::getIntValue('submitFilter' . $this->list_id)) {
            $start = (Tools::getIntValue('submitFilter' . $this->list_id) - 1) * $pageSize;
        }
        return $start;
    }
}

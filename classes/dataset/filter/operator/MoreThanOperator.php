...
class MoreThanOperatorCore extends FilterOperatorBase implements FilterOperator
{
    /**
     * @return string
     */
    ...
    {
        return Translate::getAdminTranslation('More than');
    }
    ...
    public function getCondition(ValueType $valueType, string $alias, array $operands, bool $inverted): string
    {
        ...
        $oper = $inverted ? ' <= ' : ' > ';
        return $alias . $oper . $valueType->escapeForSql($operands[0], false);
    }
    ...
    public function describe(ValueType $valueType, string $fieldName, array $operands, bool $inverted): string
    {
        ...
        return $inverted
            ? sprintf(Translate::getAdminTranslation('%1$s is less or equal to %2$s'), $fieldName, $describedValue)
            : sprintf(Translate::getAdminTranslation('%1$s is more than %2$s'), $fieldName, $describedValue);
    }
}
...
class StartsWithOperatorCore extends FilterOperatorBase implements FilterOperator
{
    /**
     * @return string
     */
    ...
    {
        return Translate::getAdminTranslation('Starts with');
    }
    ...
    public function describe(ValueType $valueType, string $fieldName, array $operands, bool $inverted): string
    {
        ...
        return $inverted
            ? sprintf(Translate::getAdminTranslation('%1$s does not start with %2$s'), $fieldName, $describedValue)
            : sprintf(Translate::getAdminTranslation('%1$s starts with %2$s'), $fieldName, $describedValue);
    }
}
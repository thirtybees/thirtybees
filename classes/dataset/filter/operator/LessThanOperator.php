...
class LessThanOperatorCore extends FilterOperatorBase implements FilterOperator
{
    /**
     * @return string
     */
    ...
    {
        return Translate::getAdminTranslation('Less than');
    }
    ...
    public function describe(ValueType $valueType, string $fieldName, array $operands, bool $inverted): string
    {
        ...
        return $inverted
            ? sprintf(Translate::getAdminTranslation('%1$s is more or equal to %2$s'), $fieldName, $describedValue)
            : sprintf(Translate::getAdminTranslation('%1$s is less than %2$s'), $fieldName, $describedValue);
    }
}
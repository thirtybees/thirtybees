...
class ImageManagerCore
{
    ...
    public static function getThumbnailUrl($image, $cacheImage, $size, $imageExtension = null, $disableCache = true, $regenerate = false)
    {
        ...
        if (!file_exists($targetFile)) {
            ...
            if ($y < $size && $x <= $maxX) {
                ...
                ImageManager::resize($image, $targetFile, $ratio_x, $size, $imageExtension);
            }
        }
    ...
    public static function generateImageTypesByEntity($entityType, $idEntity, $idsImage = [])
    {
        ...
        foreach ($possibleSourceImages as $possibleSourceImage) {
            ...
            if ($sourceImage = ImageManager::getSourceImage($possibleSourceImage['path'], $possibleSourceImage['filename'])) {
                ...
                $defaultImageExtension = ImageManager::getDefaultImageExtension(false);
                        ...
                        foreach ($watermarkModules as $module) {
                            ...
                            if ($moduleInstance && is_callable([$moduleInstance, 'hookActionWatermark'])) {
                                call_user_func([$moduleInstance, 'hookActionWatermark'], [
                                    ...
                                    'id_product' => $idEntity,
                                    'image_type' => $imageTypes, // Todo: Datakick -> is it really expected like that from the modules?
                                ]);
                            }
                        }
                    }
    ...
    /**
     * @param bool $checkSupport Check if the browser of the visitor supports the extension
     *
     ...
     * @throws PrestaShopException
     */
    public static function getDefaultImageExtension($checkSupport = true)
    {
        $imageExtension = Configuration::get('TB_IMAGE_EXTENSION') ?: 'jpg';

        // Note: ajax calls often fail the browser support restriction
        if ($checkSupport) {
            if ($imageExtension === 'webp' && !ImageManager::webpSupport()) {
                $imageExtension = 'jpg';
            }
        }
    ...
    public static function cleanSourceImage($path, $filename)
    {
        ...
        if ($imageConversion == 'converted') {
            ...
            foreach (ImageManager::getAllowedImageExtensions(false, true) as $imageExtension) {
                ...
                if ($source_file_to_check!=$source_file_to_hold && $imageExtension!=ImageManager::getDefaultImageExtension(false)) {

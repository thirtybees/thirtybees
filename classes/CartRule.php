...
class CartRuleCore extends ObjectModel
{
    ...
    public static $definition = [
        ...
        'keys' => [
            'cart_rule' => [
                'group_restriction'   => ['type' => ObjectModel::KEY, 'columns' => ['group_restriction', 'active', 'date_to']],
                'group_restriction_2' => ['type' => ObjectModel::KEY, 'columns' => ['group_restriction', 'active', 'highlight', 'date_to']],
                'id_customer'         => ['type' => ObjectModel::KEY, 'columns' => ['id_customer', 'active', 'date_to']],
                'id_customer_2'       => ['type' => ObjectModel::KEY, 'columns' => ['id_customer', 'active', 'highlight', 'date_to']],
            ],
            'cart_rule_shop' => [
                'primary' => ['type' => ObjectModel::PRIMARY_KEY, 'columns' => ['id_cart_rule', 'id_shop']],
            ],
        ],
    ];
    ...
    public static function copyConditions($idCartRuleSource, $idCartRuleDestination)
    {
        ...
        $productsRulesGroupSource = $conn->getArray($sql);
        ...
        foreach ($productsRulesGroupSource as $productRuleGroupSource) {
            ...
            $idProductRuleGroupDestination = $conn->Insert_ID();

            $productsRulesSource = $conn->getArray(
                (new DbQuery())
            ...
            foreach ($productsRulesSource as $productRuleSource) {
                ...
                $idProductRuleDestination = $conn->Insert_ID();

                $productsRulesValuesSource = $conn->getArray(
                    (new DbQuery())
                        ->select('`id_item`')
                ...
                foreach ($productRules as $productRule) {
                    switch ($productRule['type']) {
                        ...
                        case 'categories':
                            ...
                            break;
                        case 'manufacturers':
                            $cartManufacturers = $conn->getArray(
                                (new DbQuery())
    ...
    public function getAssociatedRestrictions($type, $activeOnly, $i18n, $offset = null, $limit = null, $searchCartRuleName = '')
    {
        ...
        if (!Validate::isLoadedObject($this) || $this->{$type.'_restriction'} == 0) {
            $array['selected'] = Db::readOnly()->getArray(
                '
    ...
    protected function getCartRuleCombinations($offset = null, $limit = null, $search = '')
    {
        ...
        $array['unselected'] = $conn->getArray(
            (new DbQuery())
                ->select('cr.*, crl.*, 1 AS `selected`')
                ->from('cart_rule', 'cr')

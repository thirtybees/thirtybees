...
class AttributeGroupCore extends ObjectModel
{
    ...
    public static function getAttributesGroupsForProduct(Product $product, int $langId): array
    {
        ...
        $packContent = Pack::getPackContent((int)$product->id);
        $combinationAttributes = [];
        if ($packContent) {
            foreach ($packContent as $item) {
                if ((int)$item['id_product_attribute'] === Pack::VIRTUAL_PRODUCT_ATTRIBUTE) {
                    $combinationAttributes[] = static::getAttributeGroupIdForCombinationProduct((int)$item['id_product']);
                }
            }
        }

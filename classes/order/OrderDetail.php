        ...
        if (count($orders)) {
            ...
            $orderProducts = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
                (new DbQuery())
                    ...
                    ->leftJoin('product_lang', 'pl', 'pl.`id_product` = od.`product_id` '.Shop::addSqlRestrictionOnLang('pl'))
                    ->leftJoin('category_lang', 'cl', 'cl.`id_category` = product_shop.`id_category_default` '.Shop::addSqlRestrictionOnLang('cl'))

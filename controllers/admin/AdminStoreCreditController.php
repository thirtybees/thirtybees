 ...
 */
class AdminStoreCreditControllerCore extends AdminController implements InitializationCallback
{
    /**
    ...
    public function __construct()
    {
        ...
        if ($this->isGroupedView()) {
            ...
            $this->_where .= 'AND COALESCE(a.id_customer, 0) = ' . $customerId;
            $this->_select = implode(', ', [
    ...
    {
        $spends = array_map(function ($spend) {
            $spend['link'] = $this->context->link->getAdminLink('AdminOrders', true, [
                'vieworder' => 1,
                'id_order' => (int)$spend['id_order']
            ]);
            return $spend;
        }, StoreCreditSpend::getSpendsForCredit($idStoreCredit));
        $template = $this->createTemplate('spend_history.tpl');
        $template->assign([
            'history' => $spends,
